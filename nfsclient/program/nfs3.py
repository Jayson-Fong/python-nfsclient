...
class NFSv3(RPC):
    ...
    def get_sattr3(
        ...
        attrs = sattr3(
            ...
            atime=(
                set_time(SET_TO_CLIENT_TIME, NFSTime(int(atime_s), int(atime_ns)))
                if atime_flag == SET_TO_CLIENT_TIME
                else set_time(atime_flag)
            ),
            mtime=(
                set_time(SET_TO_CLIENT_TIME, NFSTime(int(mtime_s), int(mtime_ns)))
                if mtime_flag == SET_TO_CLIENT_TIME
                else set_time(mtime_flag)
            ),
        )
        return attrs
    ...
    def getattr(self, file_handle, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: GETATTR on %s" % (NFS3_PROCEDURE_GETATTR, self.host)
        )
    ...
    def setattr(
        ...
        gid=None,
        size=None,
        atime_flag=SET_TO_SERVER_TIME,
        atime_s=None,
        atime_us=None,
        mtime_flag=SET_TO_SERVER_TIME,
        mtime_s=None,
        mtime_us=None,
        check=False,
        obj_ctime=None,
        ...
        logger.debug(
            "NFSv3 procedure %d: GETATTR on %s" % (NFS3_PROCEDURE_SETATTR, self.host)
        )
        data = self.nfs_request(
    ...
    def lookup(self, dir_handle, file_folder, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: LOOKUP on %s" % (NFS3_PROCEDURE_LOOKUP, self.host)
        )
        data = self.nfs_request(
    ...
    def access(self, file_handle, access_option, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: ACCESS on %s" % (NFS3_PROCEDURE_ACCESS, self.host)
        )
        data = self.nfs_request(
    ...
    def readlink(self, file_handle, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: READLINK on %s" % (NFS3_PROCEDURE_READLINK, self.host)
        )
        data = self.nfs_request(
    ...
    def create(
        ...
        logger.debug(
            "NFSv3 procedure %d: CREATE on %s" % (NFS3_PROCEDURE_CREATE, self.host)
        )
        data = self.nfs_request(
    ...
    def symlink(self, dir_handle, link_name, link_to_path, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: SYMLINK on %s" % (NFS3_PROCEDURE_SYMLINK, self.host)
        )
        res = self.nfs_request(
    ...
    def mknod(
        ...
        if ftype in (NF3CHR, NF3BLK):
            spec = SpecialDataV3(major=spec_major, minor=spec_minor)
            what = mknoddata3(
                type=ftype, device=devicedata3(dev_attributes=attrs, spec=spec)
            )
    ...
    def remove(self, dir_handle, file_name, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: REMOVE on %s" % (NFS3_PROCEDURE_REMOVE, self.host)
        )
        res = self.nfs_request(
    ...
    def rename(self, dir_handle_from, from_name, dir_handle_to, to_name, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: RENAME on %s" % (NFS3_PROCEDURE_RENAME, self.host)
        )
        res = self.nfs_request(
    ...
    def readdir(self, dir_handle, cookie=0, cookie_verf="0", count=4096, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: READDIR on %s" % (NFS3_PROCEDURE_READDIR, self.host)
        )
        res = self.nfs_request(
    ...
    def pathconf(self, file_handle, auth=None):
        ...
        logger.debug(
            "NFSv3 procedure %d: PATHCONF on %s" % (NFS3_PROCEDURE_PATHCONF, self.host)
        )
        data = self.nfs_request(
