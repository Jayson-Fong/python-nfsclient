...
class nfs_pro_v3Packer(Packer):
    ...
    def pack_nfsstat3(self, data):
        if data not in [
            ...
            const.NFS3ERR_ROFS,
            const.NFS3ERR_MLINK,
            const.NFS3ERR_NAMETOOLONG,
            const.NFS3ERR_NOTEMPTY,
            const.NFS3ERR_DQUOT,
            const.NFS3ERR_STALE,
            const.NFS3ERR_REMOTE,
            const.NFS3ERR_BADHANDLE,
            const.NFS3ERR_NOT_SYNC,
            const.NFS3ERR_BAD_COOKIE,
            const.NFS3ERR_NOTSUPP,
            const.NFS3ERR_TOOSMALL,
...
class nfs_pro_v3Unpacker(Unpacker):
    ...
    def unpack_post_op_attr(self, data_format="json"):
        data = types.PostOperationAttributes()
    ...
    def unpack_pre_op_attr(self, data_format="json"):
        data = types.PreOperationAttributes()
        data.present = self.unpack_bool()
        if data.present == const.TRUE:
    ...
    def unpack_post_op_fh3(self, data_format="json"):
        data = types.PostOperationFileHandleV3()
        data.present = self.unpack_bool()
        if data.present == const.TRUE:
